...
By default, any configuration you add in your own `tailwind.config.js` file is intelligently merged with the [default configuration](https://github.com/hyaliyun/tailwinds/blob/master/stubs/config.full.js), with your own configuration acting as a set of overrides and extensions.
...
By default, presets themselves extend Tailwind's [default configuration](https://github.com/hyaliyun/tailwinds/blob/master/stubs/config.full.js) just like your own configuration would. If you'd like to create a preset that completely replaces the default configuration, include an empty `presets` key in the preset itself:
